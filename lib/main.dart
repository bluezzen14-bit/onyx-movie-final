...
‎      themeMode: mode,
‎      home: AuthScreen(onTheme: (v) async {
‎        final p = await SharedPreferences.getInstance();
‎        await p.setBool('isDark', v);
‎        setState(() => mode = v? ThemeMode.dark : ThemeMode.light);
‎      }),
‎    );
‎  }
‎}
...
‎      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Wrong admin password"), backgroundColor: Colors.red));
‎      return;
‎    }
...
‎              SizedBox(height: 20),
‎              TextField(controller: emailC, style: TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Email", filled: true, fillColor: Color(0xFF1E293B), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
‎              SizedBox(height: 10),
‎              TextField(controller: passC, obscureText: true, style: TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Password", filled: true, fillColor: Color(0xFF1E293B), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
‎              SizedBox(height: 20),
‎              SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: loading? null : doAuth, style: ElevatedButton.styleFrom(backgroundColor: Color(0xFF38BDF8)), child: Text(isRegister? "REGISTER / LOGIN" : "LOGIN", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)))),
‎              SizedBox(height: 10),
‎              Text("Admins: mugabibenjamin14@gmail.com / Mugabibe+-@1\nrachetyaz900@gmail.com / Akram2007", style: TextStyle(color: Colors.white24, fontSize: 10), textAlign: TextAlign.center),
‎            ],
‎          ),
‎        ),
‎      ),
...
‎            mainAxisAlignment: MainAxisAlignment.center,
‎            children: [
‎              Text("Welcome Admin: $email", style: TextStyle(color: Colors.orange, fontSize: 12)),
‎              SizedBox(height: 20),
‎              GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => UserMainNav(onTheme: onTheme, email: email, isAdmin: true))), child: Container(height: 120, decoration: BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.circular(20), border: Border.all(color: Color(0xFF38BDF8), width: 2)), child: Center(child: Text("USER PANEL\nHome + Movies + Series + Downloads + Profile", style: TextStyle(color: Colors.white), textAlign: TextAlign.center)))),
‎              SizedBox(height: 16),
‎              GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AdminMainNav(email: email))), child: Container(height: 120, decoration: BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.orange, width: 2)), child: Center(child: Text("ADMIN PANEL\nAdd Banner 1 + Movie + 5 Casts Circle", style: TextStyle(color: Colors.white), textAlign: TextAlign.center)))),
‎            ],
‎          ),
‎        ),
‎      ),
...
‎  @override
‎  State<UserMainNav> createState() => UserMainNavState();
‎}
‎
...
‎                children: [
‎                  StreamBuilder(stream: bannerRef.onValue, builder: (c, bSnap) {
‎                    String? b64;
‎                    if (bSnap.hasData && bSnap.data!.snapshot.value!= null) {
‎                      final map = Map<String, dynamic>.from(bSnap.data!.snapshot.value as Map);
‎                      if (map.isNotEmpty) {
‎                        b64 = Map<String, dynamic>.from(map.entries.first.value as Map)['imageBase64'];
‎                      }
‎                    }
‎                    return Container(height: 180, margin: EdgeInsets.all(12), decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), color: Color(0xFF1E293B)), child: b64!= null && b64.length > 100? ClipRRect(borderRadius: BorderRadius.circular(20), child: Image.memory(base64Decode(b64.split(',').last), fit: BoxFit.cover, width: double.infinity)) : Center(child: Text("Banner 1 - Upload in Admin", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))));
‎                  }),
...
‎                  SizedBox(height: 140, child: ListView.builder(scrollDirection: Axis.horizontal, padding: EdgeInsets.symmetric(horizontal: 12), itemCount: all.isEmpty? 3 : all.length, itemBuilder: (c, i) {
‎                    if (all.isEmpty) return Container(width: 100, margin: EdgeInsets.only(right: 10), decoration: BoxDecoration(color: Color(0xFF1E3A4E), borderRadius: BorderRadius.circular(15)), child: Icon(Icons.movie, color: Colors.white));
‎                    final m = Map<String, dynamic>.from(all[i].value as Map);
‎                    final poster = m['posterBase64']?? '';
‎                    return GestureDetector(onTap: () => Navigator.push(c, MaterialPageRoute(builder: (_) => DetailPage(data: m, id: all[i].key))), child: Container(width: 110, margin: EdgeInsets.only(right: 10), decoration: BoxDecoration(color: Color(0xFF1E3A4E), borderRadius: BorderRadius.circular(15)), child: ClipRRect(borderRadius: BorderRadius.circular(15), child: poster.toString().length > 100? Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover) : Icon(Icons.movie, color: Colors.white))));
‎                  })),
‎                  Padding(padding: EdgeInsets.all(12), child: Text("Latest Series", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))),
‎                  SizedBox(height: 140, child: ListView.builder(scrollDirection: Axis.horizontal, padding: EdgeInsets.symmetric(horizontal: 12), itemCount: all.isEmpty? 3 : all.length, itemBuilder: (c, i) {
‎                    if (all.isEmpty) return Container(width: 100, margin: EdgeInsets.only(right: 10), decoration: BoxDecoration(color: Color(0xFF1E3A4E), borderRadius: BorderRadius.circular(15)), child: Icon(Icons.tv, color: Colors.white));
‎                    final m = Map<String, dynamic>.from(all[i].value as Map);
‎                    if (!(m['category']?? '').toString().toLowerCase().contains('series')) return SizedBox();
‎                    final poster = m['posterBase64']?? '';
‎                    return GestureDetector(onTap: () => Navigator.push(c, MaterialPageRoute(builder: (_) => DetailPage(data: m, id: all[i].key))), child: Container(width: 110, margin: EdgeInsets.only(right: 10), decoration: BoxDecoration(color: Color(0xFF1E3A4E), borderRadius: BorderRadius.circular(15)), child: ClipRRect(borderRadius: BorderRadius.circular(15), child: poster.toString().length > 100? Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover) : Icon(Icons.tv, color: Colors.white))));
‎                  })),
‎                  Padding(padding: EdgeInsets.all(12), child: Text("Trending Movies", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))),
‎                  SizedBox(height: 140, child: ListView.builder(scrollDirection: Axis.horizontal, padding: EdgeInsets.symmetric(horizontal: 12), itemCount: all.isEmpty? 3 : all.length, itemBuilder: (c, i) {
‎                    if (all.isEmpty) return Container(width: 100, margin: EdgeInsets.only(right: 10), decoration: BoxDecoration(color: Color(0xFF1E3A4E), borderRadius: BorderRadius.circular(15)), child: Icon(Icons.movie, color: Colors.white));
‎                    final m = Map<String, dynamic>.from(all[i].value as Map);
‎                    final poster = m['posterBase64']?? '';
...
‎      if (!snap.hasData || snap.data!.snapshot.value == null) return Center(child: Text("No Movies", style: TextStyle(color: Colors.white54)));
‎      final map = Map<String, dynamic>.from(snap.data!.snapshot.value as Map);
‎      final list = map.entries.toList();
‎      return GridView.builder(padding: EdgeInsets.all(12), gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.65, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: list.length, itemBuilder: (c, i) {
‎        final m = Map<String, dynamic>.from(list[i].value as Map);
‎        final poster = m['posterBase64']?? '';
‎        return GestureDetector(onTap: () => Navigator.push(c, MaterialPageRoute(builder: (_) => DetailPage(data: m, id: list[i].key))), child: Container(decoration: BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.circular(12)), child: poster.toString().length > 100? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover)) : Icon(Icons.movie, color: Colors.white)));
‎      });
‎    });
‎  }
‎}
...
‎  Widget build(BuildContext context) {
‎    return StreamBuilder(stream: ref.onValue, builder: (c, snap) {
‎      if (!snap.hasData || snap.data!.snapshot.value == null) return Center(child: Text("No Series", style: TextStyle(color: Colors.white54)));
‎      final map = Map<String, dynamic>.from(snap.data!.snapshot.value as Map);
‎      final list = map.entries.where((e) => (Map<String, dynamic>.from(e.value as Map)['category']?? '').toString().toLowerCase().contains('series')).toList();
‎      return GridView.builder(padding: EdgeInsets.all(12), gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.65, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: list.length, itemBuilder: (c, i) {
‎        final m = Map<String, dynamic>.from(list[i].value as Map);
‎        final poster = m['posterBase64']?? '';
...
‎  @override
‎  void initState() { super.initState(); loadAvatar(); }
...
‎  @override
‎  State<AdminMainNav> createState() => AdminMainNavState();
‎}
‎
...
‎        final m = Map<String, dynamic>.from(list[i].value as Map);
‎        return Card(color: Color(0xFF1E293B), child: ListTile(title: Text(m['title']?? '', style: TextStyle(color: Colors.white)), subtitle: Text("${m['category']?? ''}", style: TextStyle(color: Colors.white54, fontSize: 11)), trailing: IconButton(icon: Icon(Icons.delete, color: Colors.red), onPressed: () async => await ref.child(list[i].key).remove())));
‎      });
‎    });
‎  }
‎}
