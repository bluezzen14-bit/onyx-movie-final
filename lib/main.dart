      ...
      child: BackdropFilter(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white.withOpacity(0.08), Colors.white.withOpacity(0.02)]),
          ),
          child: child,
        ),
      ),
...
class _IPhoneLoaderState extends State<IPhoneLoader> with SingleTickerProviderStateMixin{
  ...
  @override void initState(){super.initState();c=AnimationController(vsync:this,duration:Duration(milliseconds:900))..repeat(reverse:true);}
  @override void dispose(){c.dispose();super.dispose();}
  @override Widget build(BuildContext context){
    return Center(child: FadeTransition(opacity: Tween(begin:0.3,end:1.0).animate(c), child: Column(mainAxisSize: MainAxisSize.min, children:[Container(width:24,height:24,decoration:BoxDecoration(color:Colors.white.withOpacity(0.06),shape:BoxShape.circle,border:Border.all(color:Colors.white.withOpacity(0.15))),child:Icon(Icons.apple,color:Colors.white.withOpacity(0.7),size:14)),SizedBox(height:6),SizedBox(width:14,height:14,child:CircularProgressIndicator(strokeWidth:1.5,color:Colors.white.withOpacity(0.6)))])));
  }
}
...
class AuthScreenState extends State<AuthScreen>{
  ...
  void doAuth() async {
    ...
    await pref.setString('user_email',e);await pref.setBool('is_admin',isAdminMail);setState(()=>loading=false);
    if(!mounted)return;
...
class HomeTabState extends State<HomeTab>{
  ...
  @override Widget build(BuildContext context){
    return SafeArea(child:Column(children:[Padding(padding:EdgeInsets.all(12),child:Row(children:[Expanded(child:LiquidGlass(radius:25, child: Container(height:50, child:TextField(onChanged:(v)=>setState(()=>q=v.toLowerCase()),style:TextStyle(color:Colors.white),decoration:InputDecoration(hintText:"Search VJ, Genre...",prefixIcon:Icon(Icons.search,color:Colors.white54),border:InputBorder.none))))),SizedBox(width:10),LiquidGlass(radius:15, child: Container(height:50,width:50, child:Icon(Icons.notifications_none,color:Colors.white54)))])),Expanded(child:StreamBuilder(stream:ref.onValue,builder:(c,snap){if(!snap.hasData){return IPhoneLoader();}List<MapEntry> all=[];if(snap.data!.snapshot.value!=null){final map=Map<String,dynamic>.from(snap.data!.snapshot.value as Map);all=map.entries.toList();}return SingleChildScrollView(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[StreamBuilder(stream:bannerRef.onValue,builder:(c,bSnap){String? b64;if(bSnap.hasData&&bSnap.data!.snapshot.value!=null){final map=Map<String,dynamic>.from(bSnap.data!.snapshot.value as Map);if(map.isNotEmpty){b64=Map<String,dynamic>.from(map.entries.first.value as Map)['imageBase64'];}}return LiquidGlass(radius:20, child: Container(height:180,margin:EdgeInsets.all(12), child:b64!=null&&b64.length>100?ClipRRect(borderRadius:BorderRadius.circular(20),child:Image.memory(base64Decode(b64.split(',').last),fit:BoxFit.cover,width:double.infinity)):Center(child:Text("Banner 1 - Upload in Admin",style:TextStyle(color:Colors.white54,fontWeight:FontWeight.bold)))));}),Padding(padding:EdgeInsets.all(12),child:Text("Latest Release",style:TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.bold))),SizedBox(height:140,child:ListView.builder(scrollDirection:Axis.horizontal,padding:EdgeInsets.symmetric(horizontal:12),itemCount:all.isEmpty?3:all.length,itemBuilder:(c,i){if(all.isEmpty){return LiquidGlass(radius:15, child: Container(width:100,margin:EdgeInsets.only(right:10), child:Icon(Icons.movie,color:Colors.white54)));}final m=Map<String,dynamic>.from(all[i].value as Map);final poster=m['posterBase64']??'';return GestureDetector(onTap:()=>Navigator.push(c,MaterialPageRoute(builder:(_)=>DetailPage(data:m,id:all[i].key))),child:LiquidGlass(radius:15, child: Container(width:110,margin:EdgeInsets.only(right:10), child:ClipRRect(borderRadius:BorderRadius.circular(15),child:poster.toString().length>100?Image.memory(base64Decode(poster.toString().split(',').last),fit:BoxFit.cover):Icon(Icons.movie,color:Colors.white54)))));})),Padding(padding:EdgeInsets.all(12),child:Text("Trending",style:TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.bold))),SizedBox(height:140,child:ListView.builder(scrollDirection:Axis.horizontal,padding:EdgeInsets.symmetric(horizontal:12),itemCount:all.isEmpty?3:all.length,itemBuilder:(c,i){if(all.isEmpty){return LiquidGlass(radius:15, child: Container(width:100,margin:EdgeInsets.only(right:10), child:Icon(Icons.movie,color:Colors.white54)));}final m=Map<String,dynamic>.from(all[i].value as Map);final poster=m['posterBase64']??'';return LiquidGlass(radius:15, child: Container(width:110,margin:EdgeInsets.only(right:10), child:ClipRRect(borderRadius:BorderRadius.circular(15),child:poster.toString().length>100?Image.memory(base64Decode(poster.toString().split(',').last),fit:BoxFit.cover):Icon(Icons.movie,color:Colors.white54))));}))]));}))]));
  }
}
class MoviesTab extends StatelessWidget{
  MoviesTab({super.key});final ref=FirebaseDatabase.instance.ref('movies');
  @override Widget build(BuildContext context){
    return SafeArea(child:Column(children:[Padding(padding:EdgeInsets.all(12),child:LiquidGlass(radius:12, child: Container(width:double.infinity,padding:EdgeInsets.all(12),child:Text("Movies Page - iOS 26 Transparent Glass",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold,fontSize:16))))),Expanded(child:StreamBuilder(stream: ref.onValue, builder: (c, snap) {if (!snap.hasData) {return IPhoneLoader();}if (snap.data!.snapshot.value == null) {return Center(child: Text("No Movies", style: TextStyle(color: Colors.white54)));}final map = Map<String, dynamic>.from(snap.data!.snapshot.value as Map);final list = map.entries.toList();return GridView.builder(padding: EdgeInsets.all(12), gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.65, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: list.length, itemBuilder: (c, i) {final m = Map<String, dynamic>.from(list[i].value as Map);final poster = m['posterBase64']?? '';return GestureDetector(onTap: () {Navigator.push(c,MaterialPageRoute(builder: (_) => DetailPage(data: m, id: list[i].key)));}, child: LiquidGlass(radius:12, child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(12)), child: poster.toString().length > 100? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover)) : Icon(Icons.movie, color: Colors.white54))));});})),
    ]));
  }
}
...
class DetailPage extends StatelessWidget{
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor: Color(0xFF020617), appBar: AppBar(backgroundColor: Color(0xFF020617), title: Text(data['title']?? '')), body: SingleChildScrollView(padding: EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [LiquidGlass(child: Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Text(data['title']?? '', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), Text("${data['vj']?? ''} - ${data['genre']?? ''}", style: TextStyle(color: Colors.white54))]))),SizedBox(height: 12),Text("Casts (5 max)", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),SizedBox(height: 8),casts.isEmpty? Text("No casts", style: TextStyle(color: Colors.white54)) : SizedBox(height: 100, child: ListView.builder(scrollDirection: Axis.horizontal, itemCount: casts.length > 5? 5 : casts.length, itemBuilder: (c, i) {final cast = Map<String, dynamic>.from(casts[i] as Map);final img = cast['imageBase64']?? '';return Container(width: 70, margin: EdgeInsets.only(right: 10), child: Column(children: [LiquidGlass(radius:30, child: CircleAvatar(radius: 25, backgroundImage: img.toString().length > 100? MemoryImage(base64Decode(img.toString().split(',').last)) : null, child: img.toString().length < 100? Icon(Icons.person) : null)), Text(cast['realName']?? '', maxLines: 1, style: TextStyle(color: Colors.white, fontSize: 10))]));})),SizedBox(height: 12),LiquidGlass(child: Padding(padding:EdgeInsets.all(12), child: Text(data['description']?? '', style: TextStyle(color: Colors.white70))))])));
  }
}
...
class DownloadsTabState extends State<DownloadsTab> with SingleTickerProviderStateMixin{
  ...
  @override Widget build(BuildContext context){return SafeArea(child:Column(children:[LiquidGlass(child: Container(margin:EdgeInsets.all(12),padding:EdgeInsets.all(12),decoration:BoxDecoration(borderRadius:BorderRadius.circular(12)),child:Row(children:[Icon(Icons.storage,color:Colors.white54),SizedBox(width:10),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text("Used: 3.2GB / 15.7GB",style:TextStyle(color:Colors.white,fontSize:12)),LinearProgressIndicator(value:0.2,color:Colors.white54),Text("Free: 12.5GB available",style:TextStyle(color:Colors.white38,fontSize:10))]))]))),TabBar(controller:tab,labelColor:Colors.white,indicatorColor:Colors.white54,tabs:[Tab(text:"Downloading"),Tab(text:"Completed")]),Expanded(child:TabBarView(controller:tab,children:[Center(child:IPhoneLoader()),Center(child:Text("No available download",style:TextStyle(color:Colors.white54)))]))]));
  }
}
...
class ProfileTabState extends State<ProfileTab>{
  final picker=ImagePicker();Uint8List? avatarBytes;bool isDownloading=false;double progress=0;String statusMsg="Ready";
  ...
  Future<void> autoUpdate() async {
    try{
      setState((){isDownloading=true;progress=0;statusMsg="Checking for update...";});
      final versionUrl="https://raw.githubusercontent.com/bluezzer/onyx-movie-final/main/version.json";
      final res=await http.get(Uri.parse(versionUrl));
      if(res.statusCode!=200){setState((){statusMsg="Failed to check version";isDownloading=false;});return;}
      final data=jsonDecode(res.body);
      final latest=data['latest_version']??"1.0.3";
      final apkUrl=data['apk_url']??"https://github.com/bluezzer/onyx-movie-final/releases";
      setState(()=>statusMsg="Latest $latest found. Downloading APK...");
      final dir=await getExternalStorageDirectory();
      final filePath="${dir!.path}/onyx_movie_$latest.apk";
      final file=File(filePath);
      final apkDirect="https://github.com/bluezzer/onyx-movie-final/releases/latest/download/app-release.apk";
      final request=await http.Client().send(http.Request('GET', Uri.parse(apkDirect)));
      ...
      await for(var chunk in request.stream){
        ...
        sink.add(chunk);
        if(total>0){setState(()=>progress=received/total);}
      }
      await sink.close();
      setState(()=>statusMsg="Downloaded! Ready to install: $filePath");
      final uri=Uri.file(filePath);
      if(await canLaunchUrl(uri)){await launchUrl(uri,mode:LaunchMode.externalApplication);}
      else {final rel=Uri.parse(apkUrl);await launchUrl(rel,mode:LaunchMode.externalApplication);}
      setState(()=>isDownloading=false);
    } catch(e){
      setState(()=>isDownloading=false);
      setState(()=>statusMsg="Error: $e - Opening releases page");
      final rel=Uri.parse("https://github.com/bluezzer/onyx-movie-final/releases");
      await launchUrl(rel,mode:LaunchMode.externalApplication);
    }
  }
  @override Widget build(BuildContext context){
    return ListView(padding:EdgeInsets.all(12),children:[Center(child:GestureDetector(onTap:pickAvatar,child:LiquidGlass(radius:45, child: CircleAvatar(radius:45,backgroundImage:avatarBytes!=null?MemoryImage(avatarBytes!):null,child:avatarBytes==null?Icon(Icons.person,size:40):null)))),Center(child:Text(widget.email,style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold))),Center(child:Text(widget.isAdmin?"ADMIN":"USER",style:TextStyle(color:widget.isAdmin?Colors.orange:Colors.white54))),SizedBox(height:20),if(widget.isAdmin)LiquidGlass(child: Container(decoration:BoxDecoration(borderRadius:BorderRadius.circular(12)),child:ListTile(title:Text("Go to ADMIN PANEL - iOS 26 Glass",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>AdminMainNav(email:widget.email)))))),SizedBox(height:12),LiquidGlass(child: Padding(padding: EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Row(children:[Icon(Icons.system_update,color:Colors.white70,size:20),SizedBox(width:8),Text("Auto Update - In-App Download",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold))]),SizedBox(height:8),Text(statusMsg,style:TextStyle(color:Colors.white54,fontSize:11)),if(isDownloading)LinearProgressIndicator(value:progress>0?progress:null,color:Colors.white70),SizedBox(height:8),SizedBox(width:double.infinity,child:ElevatedButton(onPressed:isDownloading?null:autoUpdate,style:ElevatedButton.styleFrom(backgroundColor:Colors.white.withOpacity(0.1)),child:Text(isDownloading?"Downloading ${(progress*100).toStringAsFixed(0)}%":"Check & Download Update",style:TextStyle(color:Colors.white))))]))),SizedBox(height:8),LiquidGlass(child: ListTile(leading:Icon(Icons.download,color:Colors.white70),title:Text("Download Latest APK (Fallback)",style:TextStyle(color:Colors.white)),onTap:() async {final uri=Uri.parse("https://github.com/bluezzer/onyx-movie-final/releases");if(await canLaunchUrl(uri))await launchUrl(uri,mode:LaunchMode.externalApplication);})),LiquidGlass(child: ListTile(leading:Icon(Icons.security,color:Colors.white70),title:Text("Security Biometric",style:TextStyle(color:Colors.white)),onTap:() async {final auth=LocalAuthentication();if(await auth.canCheckBiometrics){final did=await auth.authenticate(localizedReason:"Auth");if(did&&mounted){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Authenticated")));}}}))]);
  }
}
...
class AdminMainNavState extends State<AdminMainNav>{
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:Color(0xFF020617),appBar:AppBar(backgroundColor:Color(0xFF020617),title:Text("ADMIN: ${widget.email} - iOS 26 Transparent",style:TextStyle(fontSize:12,color:Colors.orange))),body:pages[idx],bottomNavigationBar:BottomNavigationBar(currentIndex:idx,backgroundColor:Color(0xFF0F172A),selectedItemColor:Colors.orange,unselectedItemColor:Colors.white54,onTap:(i)=>setState(()=>idx=i),items:[BottomNavigationBarItem(icon:Icon(Icons.dashboard),label:"Dashboard"),BottomNavigationBarItem(icon:Icon(Icons.add),label:"Add"),BottomNavigationBarItem(icon:Icon(Icons.list),label:"Manage")]));
  }
}
class AdminDashboardTab extends StatelessWidget{
  final String email;const AdminDashboardTab({super.key,required this.email});
  @override Widget build(BuildContext context){
    return ListView(padding: EdgeInsets.all(16), children: [LiquidGlass(radius:16, child: Container(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Admin Dashboard - iOS 26 Transparent PRO", style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), Text(email, style: TextStyle(color: Colors.orange)), Text("Storage: 3.2GB / 15.7GB - Transparent Glass Active", style: TextStyle(color: Colors.white54, fontSize: 12))]))),SizedBox(height: 16),LiquidGlass(radius:12, child: SizedBox(width:double.infinity, height:50, child: ElevatedButton(onPressed: () {Navigator.push(context, MaterialPageRoute(builder: (_) => AdminAddMovie()));}, style: ElevatedButton.styleFrom(backgroundColor: Colors.white.withOpacity(0.08)), child: Text("Add Banner 1 + Movie + 5 Casts - Glass", style: TextStyle(color: Colors.white))))),]);
  }
}
class AdminManageMovies extends StatelessWidget{
  ...
  @override Widget build(BuildContext context){
    return StreamBuilder(stream: ref.onValue, builder: (c, snap) {if (!snap.hasData) {return IPhoneLoader();}if (snap.data!.snapshot.value == null) {return Center(child: Text("No movies", style: TextStyle(color: Colors.white54)));}final map = Map<String, dynamic>.from(snap.data!.snapshot.value as Map);final list = map.entries.toList();return ListView.builder(padding: EdgeInsets.all(12), itemCount: list.length, itemBuilder: (c, i) {final m = Map<String, dynamic>.from(list[i].value as Map);final title = m['title']?? '';final cat = m['category']?? '';return Padding(padding: EdgeInsets.only(bottom:8), child: LiquidGlass(child: ListTile(title: Text(title, style: TextStyle(color: Colors.white)), subtitle: Text(cat, style: TextStyle(color: Colors.white54, fontSize: 11)), trailing: IconButton(icon: Icon(Icons.delete, color: Colors.redAccent), onPressed: () async {await ref.child(list[i].key).remove();}))));});});
  }
}
...
class AdminAddMovieState extends State<AdminAddMovie>{
  ...
  void save() async {
    ...
    if(mounted){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Saved! Transparent Glass PRO")));Navigator.pop(context);}
  }
  @override Widget build(BuildContext context){
    return Scaffold(backgroundColor: Color(0xFF020617), appBar: AppBar(backgroundColor: Color(0xFF020617), title:Text("ADMIN PRO: Add Movie - Transparent iOS 26")), body: SingleChildScrollView(padding: EdgeInsets.all(12), child: Column(children: [LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:titleC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Movie Title *", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:descC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Description", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:vjC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"VJ Name", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:genreC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Genre", border:InputBorder.none)))),SizedBox(height:12),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: Row(children:[Text("Category:", style:TextStyle(color:Colors.white)),SizedBox(width:10),DropdownButton<String>(value
