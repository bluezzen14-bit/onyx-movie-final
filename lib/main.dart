  ...
  @override Widget build(BuildContext context){
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Container(
          decoration: BoxDecoration(color: Colors.white.withOpacity(0.08), borderRadius: BorderRadius.circular(radius), border: Border.all(color: Colors.white.withOpacity(0.2))),
          child: child,
        ),
      ),
    );
...
class _IPhoneLoaderState extends State<IPhoneLoader> with SingleTickerProviderStateMixin{
  ...
  @override void initState(){super.initState();c=AnimationController(vsync:this,duration:Duration(milliseconds:800))..repeat(reverse:true);}
  @override void dispose(){c.dispose();super.dispose();}
  @override Widget build(BuildContext context){
    return Center(child: FadeTransition(opacity: Tween(begin:0.4,end:1.0).animate(c), child: Column(mainAxisSize: MainAxisSize.min, children:[Container(width:40,height:40,decoration:BoxDecoration(color:Colors.white.withOpacity(0.12),shape:BoxShape.circle),child:Icon(Icons.apple,color:Colors.white,size:24)),SizedBox(height:8),SizedBox(width:16,height:16,child:CircularProgressIndicator(strokeWidth:2,color:Color(0xFF38BDF8)))])));
  }
}
...
class ProfileTabState extends State<ProfileTab>{
  ...
  @override Widget build(BuildContext context){return ListView(padding:EdgeInsets.all(12),children:[Center(child:GestureDetector(onTap:pickAvatar,child:LiquidGlass(radius:45, child: CircleAvatar(radius:45,backgroundImage:avatarBytes!=null?MemoryImage(avatarBytes!):null,child:avatarBytes==null?Icon(Icons.person,size:40):null)))),Center(child:Text(widget.email,style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold))),Center(child:Text(widget.isAdmin?"ADMIN":"USER",style:TextStyle(color:widget.isAdmin?Colors.orange:Color(0xFF38BDF8)))),SizedBox(height:20),if(widget.isAdmin)LiquidGlass(child: Container(decoration:BoxDecoration(borderRadius:BorderRadius.circular(12)),child:ListTile(title:Text("Go to ADMIN PANEL - iOS 26 Glass",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>AdminMainNav(email:widget.email)))))),LiquidGlass(child: ListTile(leading:Icon(Icons.download,color:Color(0xFF38BDF8)),title:Text("Download Latest APK",style:TextStyle(color:Colors.white)),onTap:() async {final uri=Uri.parse("https://github.com/bluezzer/onyx-movie-final/releases");if(await canLaunchUrl(uri))await launchUrl(uri,mode:LaunchMode.externalApplication);})),LiquidGlass(child: ListTile(leading:Icon(Icons.security,color:Color(0xFF38BDF8)),title:Text("Security Biometric",style:TextStyle(color:Colors.white)),onTap:() async {final auth=LocalAuthentication();if(await auth.canCheckBiometrics){final did=await auth.authenticate(localizedReason:"Auth");if(did&&mounted){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Authenticated")));}}}))]);
  }
}
...
class AdminMainNavState extends State<AdminMainNav>{
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:Color(0xFF020617),appBar:AppBar(backgroundColor:Color(0xFF020617),title:Text("ADMIN: ${widget.email} - iOS 26 Glass",style:TextStyle(fontSize:12,color:Colors.orange))),body:pages[idx],bottomNavigationBar:BottomNavigationBar(currentIndex:idx,backgroundColor:Color(0xFF0F172A),selectedItemColor:Colors.orange,unselectedItemColor:Colors.white54,onTap:(i)=>setState(()=>idx=i),items:[BottomNavigationBarItem(icon:Icon(Icons.dashboard),label:"Dashboard"),BottomNavigationBarItem(icon:Icon(Icons.add),label:"Add"),BottomNavigationBarItem(icon:Icon(Icons.list),label:"Manage")]));
  }
}
class AdminDashboardTab extends StatelessWidget{
  final String email;const AdminDashboardTab({super.key,required this.email});
  @override Widget build(BuildContext context){
    return ListView(padding: EdgeInsets.all(16), children: [LiquidGlass(radius:16, child: Container(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Admin Dashboard - iOS 26 Glass PRO", style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), Text(email, style: TextStyle(color: Colors.orange)), Text("Storage: 3.2GB / 15.7GB - Liquid Glass Active", style: TextStyle(color: Colors.white54, fontSize: 12))]))),SizedBox(height: 16),LiquidGlass(radius:12, child: SizedBox(width:double.infinity, height:50, child: ElevatedButton(onPressed: () {Navigator.push(context, MaterialPageRoute(builder: (_) => AdminAddMovie()));}, style: ElevatedButton.styleFrom(backgroundColor: Colors.orange), child: Text("Add Banner 1 + Movie + 5 Casts - Glass", style: TextStyle(color: Colors.black))))),]);
  }
}
...
class AdminAddMovieState extends State<AdminAddMovie>{
  ...
  void save() async {
    ...
    if(mounted){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Saved! Liquid Glass PRO")));Navigator.pop(context);}
  }
  @override Widget build(BuildContext context){
    return Scaffold(backgroundColor: Color(0xFF020617), appBar: AppBar(title:Text("ADMIN PRO: Add Movie - iOS 26 Glass")), body: SingleChildScrollView(padding: EdgeInsets.all(12), child: Column(children: [LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:titleC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Movie Title *", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:descC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Description", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:vjC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"VJ Name", border:InputBorder.none)))),SizedBox(height:8),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: TextField(controller:genreC, style:TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"Genre", border:InputBorder.none)))),SizedBox(height:12),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: Row(children:[Text("Category:", style:TextStyle(color:Colors.white)),SizedBox(width:10),DropdownButton<String>(value:category, dropdownColor:Color(0xFF1E293B), style:TextStyle(color:Colors.white), items:["Movie","Series"].map((e)=>DropdownMenuItem(value:e,child:Text(e))).toList(),onChanged:(v)=>setState(()=>category=v!))]))),SizedBox(height:12),Row(children:[Expanded(child:LiquidGlass(child: ElevatedButton.icon(onPressed:pickPoster, style:ElevatedButton.styleFrom(backgroundColor:Colors.transparent, shadowColor:Colors.transparent), icon:Icon(Icons.image,color:Colors.white),label:Text(posterBytes==null?"Pick Poster":"Poster OK", style:TextStyle(color:Colors.white))))),SizedBox(width:8),Expanded(child:LiquidGlass(child: ElevatedButton.icon(onPressed:pickBanner, style:ElevatedButton.styleFrom(backgroundColor:Colors.transparent, shadowColor:Colors.transparent), icon:Icon(Icons.panorama,color:Colors.white),label:Text(bannerBytes==null?"Pick Banner 1":"Banner OK", style:TextStyle(color:Colors.white)))))]),SizedBox(height:12),LiquidGlass(child: Padding(padding:EdgeInsets.all(8), child: Row(children:[Text("Casts (max 5)", style:TextStyle(color:Colors.white)),Spacer(),ElevatedButton(onPressed:addCast,child:Text("Add Cast"))]))),Wrap(spacing:8,children:casts.map((c)=>Chip(label:Text(c['realName']))).toList()),SizedBox(height:20),LiquidGlass(radius:12, child: SizedBox(width:double.infinity,height:50,child:ElevatedButton(onPressed:save,style:ElevatedButton.styleFrom(backgroundColor:Color(0xFFFF8C00)),child:Text("Save to Firebase - Glass PRO",style:TextStyle(color:Colors.black,fontWeight:FontWeight.bold)))))])));
  }
}
