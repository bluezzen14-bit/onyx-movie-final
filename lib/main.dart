...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(stream: dbRef.orderByChild('timestamp').onValue, builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) { return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [iphoneLoad(), const SizedBox(height: 8), const Text("Loading...", style: TextStyle(color: Colors.white38, fontSize: 11))])); }
      List<Map> list = [];
      if (snapshot.hasData && snapshot.data!.snapshot.value!= null) { final map = snapshot.data!.snapshot.value as Map; list = map.values.map((e) => e as Map).toList(); list.sort((a, b) => (b['timestamp']?? 0).compareTo(a['timestamp']?? 0)); }
      return ListView(padding: const EdgeInsets.only(bottom: 20), children: [SizedBox(height: 190, child: PageView.builder(itemCount: list.isEmpty? 3 : (list.length > 7? 7 : list.length), itemBuilder: (_, i) { if (list.isEmpty) { return Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Glass(radius: 22, child: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [skyBlue.withOpacity(0.5), Colors.black.withOpacity(0.2)])), child: Center(child: Text("Banner ${i + 1} - Upload in Admin", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))))); } final m = list[i]; return Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: GestureDetector(onTap: () { Navigator.push(context, MaterialPageRoute(builder: (_) => PlayerPage(movie: m))); }, child: Glass(radius: 22, child: Stack(children: [Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [skyBlue.withOpacity(0.6), Colors.black.withOpacity(0.3)])), child: const Center(child: Icon(Icons.play_circle_fill_rounded, size: 50, color: Colors.white))), Positioned(bottom: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.black.withOpacity(0.8), Colors.transparent], begin: Alignment.bottomCenter, end: Alignment.topCenter)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(m['title']?? "", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), Text("${m['genre']?? ''} • ${m['year']?? ''} • ${m['vj']?? ''} • S${m['season']?? ''} E${m['episode']?? ''}", style: const TextStyle(color: skyBlue, fontSize: 10)), Text(m['description']?? "", maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white70, fontSize: 10))]))), Positioned(top: 12, right: 12, child: Glass(radius: 20, child: IconButton(icon: const Icon(Icons.favorite_border_rounded, color: Colors.white, size: 18), onPressed: () {})))])))); })), section(context, "Latest Release", list), section(context, "Latest Series", list.where((m) => m['type'] == 'series').toList()), section(context, "Trending Movies", list), section(context, "Animations", list.where((m) => m['category'] == 'Animation').toList()), section(context, "Indian Movies", list.where((m) => m['category'] == 'Indian').toList())]);
    });
  }
}

Widget section(BuildContext ctx, String title, List list) {
  if (list.isEmpty) { return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 8), child: Text(title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14))), SizedBox(height: 140, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 12), scrollDirection: Axis.horizontal, itemCount: 5, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, i) => Glass(radius: 16, child: Container(width: 100, decoration: BoxDecoration(color: skyBlue.withOpacity(0.2), borderRadius: BorderRadius.circular(16)), child: Stack(children: [const Center(child: Icon(Icons.movie_rounded, color: Colors.white)), Positioned(top: 6, right: 6, child: Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: skyBlue, borderRadius: BorderRadius.circular(6)), child: const Text("VJ Junior", style: TextStyle(fontSize: 7, color: Colors.black, fontWeight: FontWeight.bold))))]))))]); }
  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)), TextButton(onPressed: () { Navigator.push(ctx, MaterialPageRoute(builder: (_) => MoviesPage(initialList: list))); }, child: const Text("See more", style: TextStyle(color: skyBlue, fontSize: 11)))])), SizedBox(height: 150, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 12), scrollDirection: Axis.horizontal, itemCount: list.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, i) { final m = list[i]; return GestureDetector(onTap: () { Navigator.push(ctx, MaterialPageRoute(builder: (_) => PlayerPage(movie: m))); }, child: Glass(radius: 16, child: Container(width: 110, decoration: BoxDecoration(color: skyBlue.withOpacity(0.25), borderRadius: BorderRadius.circular(16)), child: Stack(children: [const Center(child: Icon(Icons.play_circle_fill_rounded, color: Colors.white)), Positioned(top: 6, right: 6, child: Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: skyBlue, borderRadius: BorderRadius.circular(6)), child: Text(m['vj']?? "VJ", style: const TextStyle(fontSize: 7, color: Colors.black, fontWeight: FontWeight.bold)))), Positioned(bottom: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: Colors.black.withOpacity(0.6), borderRadius: const BorderRadius.vertical(bottom: Radius.circular(16))), child: Text(m['title']?? "", maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))))]))); }))]);
}
...
class _ProfilePageState extends State<ProfilePage> {
  String latestVer = ""; String whatsNew = ""; String apkUrl = "";
  @override
  ...
  Future<void> checkVersion() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (latestVer!= "" && latestVer!= appVer) { if (mounted) { showDialog(context: context, builder: (_) => AlertDialog(backgroundColor: const Color(0xFF0F172A), title: Text("New Update $latestVer Available!", style: const TextStyle(color: skyBlue)), content: Text(whatsNew, style: const TextStyle(color: Colors.white70)), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text("Later")), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: skyBlue, foregroundColor: Colors.black), onPressed: () async { final uri = Uri.parse(apkUrl.isEmpty? "https://github.com/bluezzer/onyx-movie-final/releases" : apkUrl); if (await canLaunchUrl(uri)) { await launchUrl(uri, mode: LaunchMode.externalApplication); } }, child: const Text("UPDATE NOW"))])); } }
      }
    } catch (e) {}
  }
  @override
  Widget build(BuildContext context) {
    return ListView(padding: const EdgeInsets.all(16), children: [FutureBuilder<SharedPreferences>(future: SharedPreferences.getInstance(), builder: (c, s) { final email = s.data?.getString('user')?? "guest@onyx.com"; return Glass(child: ListTile(leading: const CircleAvatar(backgroundColor: skyBlue, child: Icon(Icons.person_rounded, color: Colors.black)), title: Text(email, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), subtitle: const Text("ONYX Member", style: TextStyle(color: skyBlue, fontSize: 10)))); }), const SizedBox(height: 12), Glass(child: Column(children: [ListTile(title: Text(latestVer == "" || latestVer == appVer? "App Up To Date" : "New Version $latestVer Available!", style: const TextStyle(color: Colors.white, fontSize: 13)), subtitle: Text(latestVer == ""? "Current v$appVer - Checking..." : (latestVer == appVer? "Current v$appVer - Latest" : "$whatsNew\nTap to update"), style: const TextStyle(color: Colors.white38, fontSize: 10)), trailing: const Icon(Icons.system_update_rounded, color: skyBlue), onTap: () async { await checkVersion(); final uri = Uri.parse(apkUrl.isEmpty? "https://github.com/bluezzer/onyx-movie-final/releases" : apkUrl); if (await canLaunchUrl(uri)) { await launchUrl(uri, mode: LaunchMode.externalApplication); } }), const Divider(color: Colors.white10), SwitchListTile(value: true, onChanged: (v) {}, title: const Text("Download to Gallery", style: TextStyle(color: Colors.white, fontSize: 13)), activeColor: skyBlue), SwitchListTile(value: false, onChanged: (v) {}, title: const Text("Play Offline", style: TextStyle(color: Colors.white, fontSize: 13)), activeColor: skyBlue), ListTile(title: const Text("Rate Us", style: TextStyle(color: Colors.white, fontSize: 13)), trailing: const Icon(Icons.star_rounded, color: skyBlue), onTap: () {}), ListTile(title: const Text("Share App", style: TextStyle(color: Colors.white, fontSize: 13)), trailing: const Icon(Icons.share_rounded, color: skyBlue), onTap: () { Share.share("Download ONYX MOVIES Sky Blue Liquid Glass https://github.com/bluezzer/onyx-movie-final"); })])), const SizedBox(height: 12), const Text("Settings", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Glass(child: Column(children: [SwitchListTile(value: true, onChanged: (v) {}, title: const Text("Dark / Light Mode", style: TextStyle(color: Colors.white, fontSize: 13)), activeColor: skyBlue), SwitchListTile(value: true, onChanged: (v) {}, title: const Text("High Speed Downloads", style: TextStyle(color: Colors.white, fontSize: 13)), activeColor: skyBlue), SwitchListTile(value: true, onChanged: (v) {}, title: const Text("Allow All Permissions", style: TextStyle(color: Colors.white, fontSize: 13)), activeColor: skyBlue), const ListTile(title: Text("Security - 6 Digit Passcode", style: TextStyle(color: Colors.white, fontSize: 13)), trailing: Icon(Icons.lock_rounded, color: skyBlue)), const ListTile(title: Text("Biometric", style: TextStyle(color: Colors.white, fontSize: 13)), trailing: Icon(Icons.fingerprint_rounded, color: skyBlue)), const ListTile(title: Text("Face ID", style: TextStyle(color: Colors.white, fontSize: 13)), trailing: Icon(Icons.face_rounded, color: skyBlue))])), const SizedBox(height: 12), Glass(child: Column(children: [const ListTile(title: Text("Contact Us", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))), ListTile(title: const Text("mugabibenjamin14@gmail.com", style: TextStyle(color: Colors.white70, fontSize: 12)), leading: const Icon(Icons.email_rounded, color: skyBlue, size: 18), onTap: () async { final uri = Uri.parse("mailto:mugabibenjamin14@gmail.com"); if (await canLaunchUrl(uri)) { await launchUrl(uri); } }), const ListTile(title: Text("Instagram: onyxmovies", style: TextStyle(color: Colors.white70, fontSize: 12)), leading: Icon(Icons.camera_alt_rounded, color: skyBlue, size: 18)), const ListTile(title: Text("Facebook: onyx movies14", style: TextStyle(color: Colors.white70, fontSize: 12)), leading: Icon(Icons.facebook_rounded, color: skyBlue, size: 18)), const ListTile(title: Text("X / Twitter: onyxmovies", style: TextStyle(color: Colors.white70, fontSize: 12)), leading: Icon(Icons.alternate_email_rounded, color: skyBlue, size: 18))]))]);
  }
}

// ===== ADMIN LOGIN WITH 3 EMAILS =====
...
class _AdminLoginState extends State<AdminLogin> {
  ...
  login() async {
    ...
    if (allowedAdmins.contains(email) || email.contains("admin") || email.contains("onyx")) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Access Denied: $email not admin. Allowed: ${allowedAdmins.join(', ')}")));
    }
    setState(() => load = false);
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF020617),
      appBar: AppBar(backgroundColor: skyBlue, title: const Text("Admin Login - 3 Emails", style: TextStyle(color: Colors.black, fontSize: 14))),
      body: Padding(
        padding: const EdgeInsets.all(24),
        child: Glass(
          radius: 20,
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.admin_panel_settings_rounded, size: 60, color: skyBlue),
              const SizedBox(height: 12),
              const Text("3 ADMINS ALLOWED", style: TextStyle(color: Colors.white38, fontSize: 11)),
              const SizedBox(height: 12),
              TextField(controller: emailC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Admin Email", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
              const SizedBox(height: 10),
              TextField(controller: passC, obscureText: true, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Password (any for now)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
              const SizedBox(height: 16),
              SizedBox(width: double.infinity, height: 50, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: skyBlue), onPressed: load? null : login, child: load? iphoneLoad() : const Text("LOGIN AS ADMIN", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)))),
              const SizedBox(height: 8),
              const Text("Allowed: mugabibenjamin14@gmail.com\nonyxmovies@gmail.com\nbluezzer@gmail.com\nor any email with admin/onyx", style: TextStyle(color: Colors.white24, fontSize: 10), textAlign: TextAlign.center),
            ]),
          ),
        ),
      ),
...
class _AdminPanelState extends State<AdminPanel> with SingleTickerProviderStateMixin {
  ...
  String posterPath = "No poster selected - Pick from Gallery";
  String videoPath = "No video selected - Pick from Gallery";
  ...
  loadCategories() {
    FirebaseDatabase.instance.ref().child('categories').onValue.listen((e) {
      if (e.snapshot.value!= null) {
        final map = e.snapshot.value as Map;
        setState(() => categories = map.values.map((v) => v.toString()).toList());
      }
    });
  }

  pickPoster() {
    setState(() => posterPath = "Poster picked from Gallery ✅ - Now add Poster URL below or it will be uploaded");
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Poster Gallery: In real app, ImagePicker opens. For now add Poster URL")));
  }

  pickVideo() {
    setState(() => videoPath = "Video picked from Gallery ✅ - Now add Video URL below");
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Video Gallery: In real app, FilePicker opens. For now add Drive/Video URL")));
  }

  upload() async {
    if (titleC.text.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Add Title first"))); return; }
    ...
    await ref.set({
      ...
      'type': type,
      'category': category,
      'poster': posterUrlC.text,
      'posterUrl': posterUrlC.text,
      'videoUrl': videoUrlC.text,
      'video': videoUrlC.text,
      'season': type == 'series'? seasonC.text : "",
      'episode': type == 'series'? episodeC.text : "",
      'downloads': 0,
      'views': 0,
      'timestamp': DateTime.now().millisecondsSinceEpoch,
    });
    setState(() => uploading = false);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Uploaded! User Home will see it instantly with Poster & Season")));
  }

  addCategory() {
    if (newCat.isEmpty) return;
    FirebaseDatabase.instance.ref().child('categories').push().set(newCat);
    setState(() => newCat = "");
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF020617),
      appBar: AppBar(backgroundColor: skyBlue, title: const Text("Admin Dashboard 100% PRO", style: TextStyle(color: Colors.black, fontSize: 14)), bottom: TabBar(controller: tabC, labelColor: Colors.black, unselectedLabelColor: Colors.black54, indicatorColor: Colors.black, tabs: const [Tab(text: "Dashboard"), Tab(text: "Movie"), Tab(text: "Series"), Tab(text: "Categories")])),
      body: TabBarView(controller: tabC, children: [
        ...
        StreamBuilder(stream: FirebaseDatabase.instance.ref().child('movies').onValue, builder: (c, snap) {
          int movies = 0, series = 0, totalDownloads = 0;
          if (snap.hasData && snap.data!.snapshot.value!= null) { final map = snap.data!.snapshot.value as Map; movies = map.values.where((m) => (m as Map)['type']!= 'series').length; series = map.values.where((m) => (m as Map)['type'] == 'series').length; for (var v in map.values) { totalDownloads += ((v as Map)['downloads']?? 0) as int; } }
          return ListView(padding: const EdgeInsets.all(16), children: [Row(children: [Expanded(child: Glass(child: Container(padding: const EdgeInsets.all(16), child: Column(children: [const Icon(Icons.movie_rounded, color: skyBlue), Text("$movies", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), const Text("Movies", style: TextStyle(color: Colors.white54, fontSize: 11))])))), const SizedBox(width: 12), Expanded(child: Glass(child: Container(padding: const EdgeInsets.all(16), child: Column(children: [const Icon(Icons.tv_rounded, color: skyBlue), Text("$series", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), const Text("Series", style: TextStyle(color: Colors.white54, fontSize: 11))]))))]), const SizedBox(height: 12), Row(children: [Expanded(child: Glass(child: Container(padding: const EdgeInsets.all(16), child: Column(children: [const Icon(Icons.download_rounded, color: skyBlue), Text("$totalDownloads", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), const Text("Total Downloads", style: TextStyle(color: Colors.white54, fontSize: 11))])))), const SizedBox(width: 12), Expanded(child: Glass(child: Container(padding: const EdgeInsets.all(16), child: const Column(children: [Icon(Icons.people_rounded, color: skyBlue), Text("850+", style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), Text("Users / Views", style: TextStyle(color: Colors.white54, fontSize: 11))]))))]), const SizedBox(height: 16), const Text("Admin Features:", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Glass(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("• Poster from Gallery + URL", style: TextStyle(color: Colors.white70, fontSize: 12)), const Text("• Video from Gallery + Drive Link", style: TextStyle(color: Colors.white70, fontSize: 12)), const Text("• Actors / Casts field for all", style: TextStyle(color: Colors.white70, fontSize: 12)), const Text("• Season / Episode for Series", style: TextStyle(color: Colors.white70, fontSize: 12)), const Text("• Category Add / Remove", style: TextStyle(color: Colors.white70, fontSize: 12)), const Text("• 3 Admin Emails Login", style: TextStyle(color: Colors.white70, fontSize: 12))])) )]);
        }),
        // MOVIE
        ListView(padding: const EdgeInsets.all(16), children: [const Text("Upload Movie - Poster & Video from Gallery", style: TextStyle(color: skyBlue, fontWeight: FontWeight.bold)), const SizedBox(height: 12), Glass(child: ListTile(title: Text(posterPath, style: const TextStyle(color: Colors.white70, fontSize: 12)), trailing: const Icon(Icons.image_rounded, color: skyBlue), onTap: pickPoster, subtitle: const Text("Tap to pick poster from gallery", style: TextStyle(color: Colors.white24, fontSize: 10)))), const SizedBox(height: 8), TextField(controller: posterUrlC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Poster URL (from gallery link or https)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), Glass(child: ListTile(title: Text(videoPath, style: const TextStyle(color: Colors.white70, fontSize: 12)), trailing: const Icon(Icons.video_file_rounded, color: skyBlue), onTap: pickVideo, subtitle: const Text("Tap to pick video from gallery", style: TextStyle(color: Colors.white24, fontSize: 10)))), const SizedBox(height: 8), TextField(controller: videoUrlC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Video URL / Google Drive Link", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 12), TextField(controller: titleC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Movie Title", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), Row(children: [Expanded(child: TextField(controller: vjC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "VJ Name who translated", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))), const SizedBox(width: 8), Expanded(child: DropdownButtonFormField(value: categories.contains(category)? category : categories.first, decoration: InputDecoration(labelText: "Category", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))), items: categories.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (v) => setState(() => category = v!)))]), const SizedBox(height: 8), TextField(controller: castsC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Actors / Casts who acted (e.g. John, Emma, Chris)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: descC, maxLines: 3, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Description / Story of Movie", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), Row(children: [Expanded(child: TextField(controller: genreC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Genre", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))), const SizedBox(width: 8), Expanded(child: TextField(controller: yearC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Year", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))))]), const SizedBox(height: 16), SizedBox(height: 52, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: skyBlue), onPressed: uploading? null : () { type = 'movie'; upload(); }, child: uploading? Row(mainAxisAlignment: MainAxisAlignment.center, children: [iphoneLoad(), const SizedBox(width: 8), const Text("Uploading...", style: TextStyle(color: Colors.black))]) : const Text("UPLOAD MOVIE - WITH ACTORS & POSTER", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))))]),
        // SERIES
        ListView(padding: const EdgeInsets.all(16), children: [const Text("Upload Series - Select Season & Actors", style: TextStyle(color: skyBlue, fontWeight: FontWeight.bold)), const SizedBox(height: 12), Row(children: [Expanded(child: DropdownButtonFormField(value: seasonC.text, decoration: InputDecoration(labelText: "Season", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))), items: List.generate(20, (i) => DropdownMenuItem(value: "${i + 1}", child: Text("Season ${i + 1}"))), onChanged: (v) => seasonC.text = v!)), const SizedBox(width: 8), Expanded(child: DropdownButtonFormField(value: episodeC.text, decoration: InputDecoration(labelText: "Episode", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))), items: List.generate(50, (i) => DropdownMenuItem(value: "${i + 1}", child: Text("Ep ${i + 1}"))), onChanged: (v) => episodeC.text = v!))]), const SizedBox(height: 12), TextField(controller: titleC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Series Title (e.g. Breaking Bad)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: castsC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Actors / Casts in Series (e.g. Bryan, Aaron)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: descC, maxLines: 3, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Series Description / Story", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: posterUrlC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Poster URL from Gallery", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: videoUrlC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Episode Video URL / Drive Link", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8), TextField(controller: vjC, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "VJ Name for Series", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 16), SizedBox(height: 52, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: skyBlue), onPressed: uploading? null : () { type = 'series'; upload(); }, child: const Text("UPLOAD SERIES - SEASON & ACTORS", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))))]),
        // CATEGORIES
        Padding(padding: const EdgeInsets.all(16), child: Column(children: [Row(children: [Expanded(child: TextField(onChanged: (v) => newCat = v, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "New Category Name (e.g. K-Drama)", border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))), const SizedBox(width: 8), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: skyBlue), onPressed: () { if (newCat.isNotEmpty) { FirebaseDatabase.instance.ref().child('categories').push().set(newCat); setState(() => newCat = ""); } }, child: const Text("Add", style: TextStyle(color: Colors.black)))]), const SizedBox(height: 16), Expanded(child: ListView.builder(itemCount: categories.length, itemBuilder: (_, i) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Glass(child: ListTile(title: Text(categories[i], style: const TextStyle(color: Colors.white)), trailing: IconButton(icon: const Icon(Icons.delete_rounded, color: Colors.redAccent, size: 20), onPressed: () async { final snap = await FirebaseDatabase.instance.ref().child('categories').get(); for (var c in snap.children) { if (c.value == categories[i]) { c.ref.remove(); } } }))))))]))
      ]),
    );
  }
}
