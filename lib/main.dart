...
// TRUE iPhone loader like your image
  ...
  @override Widget build(BuildContext context){
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        if(widget.text!=null)...[SizedBox(height: 10), Text(widget.text!, style: TextStyle(color: Colors.white70, fontSize: 12), textAlign: TextAlign.center)],
      ]),
    );
  }
}
...
class MyAppState extends State<MyApp>{
  ...
  loadTheme() async {
    ...
    setState((){
      ...
      savedEmail=p.getString('user_email');
    });
  }
  @override Widget build(BuildContext context){
    return MaterialApp(
      ...
      themeMode:mode,
      home: savedEmail!=null && savedEmail!.isNotEmpty
       ? UserMainNav(onTheme:(isDark) async {final p=await SharedPreferences.getInstance();await p.setBool('isDark',isDark);setState(()=>mode=isDark?ThemeMode.dark:ThemeMode.light);}, email:savedEmail!, isAdmin: false)
        : AuthScreen(onTheme:(isDark) async {final p=await SharedPreferences.getInstance();await p.setBool('isDark',isDark);setState(()=>mode=isDark?ThemeMode.dark:ThemeMode.light);}),
    );
  }
}
...
class AuthScreenState extends State<AuthScreen>{
  ...
  _loadSavedEmail() async {final p=await SharedPreferences.getInstance();final e=p.getString('user_email');if(e!=null) setState(()=>emailC.text=e);}
  void doAuth() async {
    ...
    await pref.setInt('registered_at', DateTime.now().millisecondsSinceEpoch);
    List<String> allEmails = pref.getStringList('all_registered_emails')?? [];
    if(!allEmails.contains(e)){allEmails.add(e);await pref.setStringList('all_registered_emails', allEmails);}
...
class ProfileTabState extends State<ProfileTab>{
  ...
  Future<void> autoUpdate() async {
    try{
      ...
      final latest=data['latest_version']??"1.0.4";
      ...
      await for(var chunk in request.stream){received+=chunk.length;sink.add(chunk);if(total>0){setState(()=>progress=received/total);}}
      await sink.close();
      setState(()=>statusMsg="Downloaded! Ready to install: $filePath");
      final uri=Uri.file(filePath);
      if(await canLaunchUrl(uri)){await launchUrl(uri,mode:LaunchMode.externalApplication);} else {final rel=Uri.parse(apkUrl);await launchUrl(rel,mode:LaunchMode.externalApplication);}
      setState(()=>isDownloading=false);
    } catch(e){setState(()=>isDownloading=false);setState(()=>statusMsg="Error: $e - Opening releases page");final rel=Uri.parse("https://github.com/bluezzer/onyx-movie-final/releases");await launchUrl(rel,mode:LaunchMode.externalApplication);}
  }
  @override Widget build(BuildContext context){
    return SafeArea(
      child: ListView(padding:EdgeInsets.all(12), children:[
        Center(child: Padding(padding: EdgeInsets.only(top: 10, bottom: 20), child: Text("Profile", style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 1.2)))),
        Center(child: LiquidGlass(radius: 15, child: Container(padding: EdgeInsets.symmetric(horizontal: 20, vertical: 12), child: Column(children: [Icon(Icons.email, color: Color(0xFF38BDF8), size: 20),SizedBox(height: 6),Text(savedEmailPermanent.isEmpty? widget.email : savedEmailPermanent, style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold, fontSize: 14), textAlign: TextAlign.center),Text("Permanently Saved", style: TextStyle(color: Colors.white38, fontSize: 10)),]),),),
        SizedBox(height: 20),
        ...
        Center(child:Text(widget.isAdmin?"ADMIN":"USER",style:TextStyle(color:widget.isAdmin?Colors.orange:Colors.white54, fontSize: 12))),
        SizedBox(height:20),
        if(widget.isAdmin)LiquidGlass(child: Container(decoration:BoxDecoration(borderRadius:BorderRadius.circular(12)),child:ListTile(title:Text("Go to ADMIN PANEL - iOS 26 Glass",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>AdminMainNav(email:widget.email)))))),
        SizedBox(height:12),
        LiquidGlass(child: Padding(padding: EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[Row(children:[Icon(Icons.system_update,color:Colors.white70,size:20),SizedBox(width:8),Text("Auto Update - In-App Download",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold))]),SizedBox(height:8),Text(statusMsg,style:TextStyle(color:Colors.white54,fontSize:11)),if(isDownloading)LinearProgressIndicator(value:progress>0?progress:null,color:Colors.white70),SizedBox(height:8),SizedBox(width:double.infinity,child:ElevatedButton(onPressed:isDownloading?null:autoUpdate,style:ElevatedButton.styleFrom(backgroundColor:Colors.white.withOpacity(0.1)),child:Text(isDownloading?"Downloading ${(progress*100).toStringAsFixed(0)}%":"Check & Download Update",style:TextStyle(color:Colors.white))))]))),
        SizedBox(height:8),
        LiquidGlass(child: ListTile(leading:Icon(Icons.download,color:Colors.white70),title:Text("Download Latest APK (Fallback)",style:TextStyle(color:Colors.white)),onTap:() async {final uri=Uri.parse("https://github.com/bluezzer/onyx-movie-final/releases");if(await canLaunchUrl(uri))await launchUrl(uri,mode:LaunchMode.externalApplication);})),
        ...
        SizedBox(height: 12),
        Center(child: Text("Email permanently saved: ${savedEmailPermanent.isEmpty? widget.email : savedEmailPermanent}", style: TextStyle(color: Colors.white24, fontSize: 9), textAlign: TextAlign.center)),
      ]),
    );
  }
}
