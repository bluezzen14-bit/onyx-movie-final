...
‎      final sb=SnackBar(content: Text("Wrong admin password"));
...
‎              onTap: () {
‎                Navigator.push(c, MaterialPageRoute(builder: (_) => DetailPage(data: m, id: list[i].key)));
‎              },
‎              child: Container(
‎                decoration: BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.circular(12)),
‎                child: poster.toString().length > 100? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover)) : Icon(Icons.movie, color: Colors.white),
‎              ),
‎            );
‎          },
‎        );
...
‎              child: Container(
‎                decoration: BoxDecoration(color: Color(0xFF1E293B), borderRadius: BorderRadius.circular(12)),
‎                child: poster.toString().length > 100? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode(poster.toString().split(',').last), fit: BoxFit.cover)) : Icon(Icons.tv, color: Colors.white),
‎              ),
‎            );
‎          },
‎        );
...
‎          children: [
‎            Text(data['title']?? '', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
‎            Text("${data['vj']?? ''} - ${data['genre']?? ''}", style: TextStyle(color: Colors.white54)),
‎            SizedBox(height: 12),
‎            Text("Casts (5 max)", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
‎            SizedBox(height: 8),
‎            casts.isEmpty? Text("No casts", style: TextStyle(color: Colors.white54)) : SizedBox(
‎              height: 100,
...
‎            ),
‎            SizedBox(height: 12),
‎            Text(data['description']?? '', style: TextStyle(color: Colors.white70)),
‎          ],
‎        ),
‎      ),
‎    );
...
‎              Text("Storage: 3.2GB / 15.7GB Used - Free 12.5GB", style: TextStyle(color: Colors.white54, fontSize: 12)),
‎            ],
‎          ),
‎        ),
‎        SizedBox(height: 16),
‎        ElevatedButton(
‎          onPressed: () {
‎            Navigator.push(context, MaterialPageRoute(builder: (_) => AdminAddMovie()));
‎          },
‎          style: ElevatedButton.styleFrom(backgroundColor: Colors.orange),
‎          child: Text("Add Banner 1 + Movie + 5 Casts", style: TextStyle(color: Colors.black)),
‎        ),
‎      ],
‎    );
‎  }
...
‎                subtitle: Text(cat, style: TextStyle(color: Colors.white54, fontSize: 11)),
‎                trailing: IconButton(
...
‎  void addCast() async {
‎    if(casts.length>=5){
‎      final sb=SnackBar(content: Text("Max 5 casts"));
‎      ScaffoldMessenger.of(context).showSnackBar(sb);
‎      return;
‎    }
...
‎                  }
‎                },
‎                child: CircleAvatar(radius:30,backgroundImage:castImg!=null?MemoryImage(castImg!):null,child:castImg==null?Icon(Icons.person):null),
‎              ),
...
‎            TextButton(
‎              onPressed:(){
‎                setState(()=>casts.add({"realName":realC.text,"actedName":actedC.text,"imageBase64":castImg!=null?base64Encode(castImg!):""}));
...
‎      final s=SnackBar(content: Text("Title required"));
‎      ScaffoldMessenger.of(context).showSnackBar(s);
‎      return;
‎    }
‎    if(bannerBytes!=null){
‎      final bRef=FirebaseDatabase.instance.ref('banners').push();
‎      await bRef.set({"imageBase64":base64Encode(bannerBytes!), "timestamp":DateTime.now().millisecondsSinceEpoch});
‎    }
...
‎    if(mounted){
‎      final s=SnackBar(content: Text("Saved to Firebase!"));
‎      ScaffoldMessenger.of(context).showSnackBar(s);
‎      Navigator.pop(context);
‎    }
‎  }
...
‎            SizedBox(height:12),
‎            Row(children:[Expanded(child:ElevatedButton.icon(onPressed:pickPoster,icon:Icon(Icons.image),label:Text(posterBytes==null?"Pick Poster":"Poster OK"))),SizedBox(width:8),Expanded(child:ElevatedButton.icon(onPressed:pickBanner,icon:Icon(Icons.panorama),label:Text(bannerBytes==null?"Pick Banner 1":"Banner OK")))]),
‎            SizedBox(height:12),
...
‎            SizedBox(height:20),
‎            SizedBox(width:double.infinity,child:ElevatedButton(onPressed:save,style:ElevatedButton.styleFrom(backgroundColor:Colors.orange),child:Text("Save to Firebase",style:TextStyle(color:Colors.black,fontWeight:FontWeight.bold)))),
‎          ],
‎        ),
‎      ),
‎    );
