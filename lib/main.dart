...
class HomeTabState extends State<HomeTab>{
  ...
  Widget sectionHeader(String title, List<MapEntry> list){
    return Padding(padding: EdgeInsets.symmetric(horizontal:12, vertical:8), child: Row(children: [Text(title, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),Spacer(),GestureDetector(onTap: ()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>SeeMorePage(title:title, movies:list))), child: Text("See More", style: TextStyle(color: Color(0xFF38BDF8), fontSize: 12)))]));
  }
  Widget moviePosterCard(Map<String,dynamic> m, String id){
    final poster=m['posterBase64']??'';final vj=m['vj']??'';
    return GestureDetector(onTap: ()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>DetailPage(data:m,id:id))), child: Container(width: 120, margin: EdgeInsets.only(right:10), child: Stack(children: [
