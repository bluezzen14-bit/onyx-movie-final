...
// --- MOVIE MODEL + VJ NAME (NEW UPDATE) ---
class Movie {
  final String title;
  final String category;
  final String year;
  final String videoUrl;
  final String vjName; // NEW: VJ Name
  final Color color;
  Movie(this.title, this.category, this.year, this.videoUrl, this.vjName, this.color);
}

// EASY UPDATE: Just add new movie with VJ name
...
// VJ LIST - Admin can upload new VJs
final List<String> vjList = ['VJ Junior', 'VJ Ice P', 'VJ Little', 'VJ Emmy', 'VJ Jingo', 'VJ Mox'];

// --- USER PANEL WITH VJ NAME ON TOP RIGHT CORNER OF BANNER ---
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 45, child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 16), itemCount: categories.length, itemBuilder: (context, i) { final cat = categories[i]; final isSel = cat == selectedCategory; return GestureDetector(onTap: () => setState(() => selectedCategory = cat), child: Container(margin: const EdgeInsets.only(right: 10), padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), decoration: BoxDecoration(color: isSel? const Color(0xFF9C27FF) : Colors.white.withOpacity(0.08), borderRadius: BorderRadius.circular(20), border: Border.all(color: isSel? Colors.transparent : Colors.white.withOpacity(0.1))), child: Center(child: Text(cat, style: TextStyle(fontWeight: isSel? FontWeight.bold : FontWeight.normal))))); })),
              ...
              Expanded(child: GridView.builder(padding: const EdgeInsets.all(16), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.72, crossAxisSpacing: 12, mainAxisSpacing: 12), itemCount: filtered.length, itemBuilder: (context, i) { final movie = filtered[i]; return GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PlayerScreen(movie: movie))), child: Container(decoration: BoxDecoration(color: Colors.white.withOpacity(0.06), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.08))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Stack(children: [Container(decoration: BoxDecoration(color: movie.color.withOpacity(0.5), borderRadius: const BorderRadius.vertical(top: Radius.circular(20))), child: Center(child: Icon(Icons.play_circle_fill_rounded, size: 50, color: Colors.white.withOpacity(0.9)))), 
              // VJ NAME ON TOP RIGHT CORNER OF BANNER
              Positioned(top: 8, right: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFF9C27FF), borderRadius: BorderRadius.circular(8), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.5), blurRadius: 4)]), child: Text(movie.vjName, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.white)))),
...
// --- ADMIN PANEL WITH UPLOAD VJ ON BANNER ---
...
class _AdminPanelState extends State<AdminPanel> {
  ...
  void addMovie() {
    if (titleCtrl.text.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter title'))); return; }
    setState(() { allMovies.add(Movie(titleCtrl.text, selectedCat, '2024', 'https://sample.mp4', selectedVJ, Colors.primaries[allMovies.length % Colors.primaries.length])); titleCtrl.text=''; });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Added ${titleCtrl.text} with $selectedVJ')));
  }
  void addVJ() {
    if (vjCtrl.text.isEmpty) return;
    setState(() { vjList.add(vjCtrl.text); selectedVJ = vjCtrl.text; vjCtrl.clear(); });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('VJ Uploaded: $selectedVJ')));
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('ADMIN: ${widget.email}'), backgroundColor: const Color(0xFF9C27FF)),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        const Text('UPLOAD VJs ON BANNER', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF9C27FF))),
        const SizedBox(height: 10),
        Row(children: [Expanded(child: TextField(controller: vjCtrl, decoration: InputDecoration(hintText: 'New VJ name e.g. VJ Sammy', filled: true, fillColor: Colors.white10, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)))), const SizedBox(width: 10), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF9C27FF)), onPressed: addVJ, child: const Text('ADD VJ'))]),
        const SizedBox(height: 10),
        Wrap(spacing: 8, children: vjList.map((vj) => Chip(label: Text(vj, style: const TextStyle(fontSize: 12)), backgroundColor: vj==selectedVJ? const Color(0xFF9C27FF) : Colors.white10, labelStyle: TextStyle(color: vj==selectedVJ? Colors.white : Colors.white70))).toList()),
        const Divider(height: 30, color: Colors.white10),
        const Text('UPLOAD MOVIE WITH VJ ON BANNER', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        TextField(controller: titleCtrl, decoration: InputDecoration(hintText: 'Movie Title', filled: true, fillColor: Colors.white10, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
        const SizedBox(height: 12),
        DropdownButtonFormField<String>(value: selectedVJ, decoration: InputDecoration(filled: true, fillColor: Colors.white10, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), labelText: 'Select VJ for Banner Top Right'), items: vjList.map((vj) => DropdownMenuItem(value: vj, child: Text(vj))).toList(), onChanged: (v) => setState(()=>selectedVJ=v!)),
        const SizedBox(height: 12),
        DropdownButtonFormField<String>(value: selectedCat, decoration: InputDecoration(filled: true, fillColor: Colors.white10, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), labelText: 'Category'), items: categories.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(()=>selectedCat=v!)),
        const SizedBox(height: 12),
        SizedBox(width: double.infinity, height: 50, child: ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF9C27FF)), onPressed: addMovie, icon: const Icon(Icons.upload), label: Text('UPLOAD MOVIE WITH $selectedVJ ON BANNER'))),
        const Divider(height: 30, color: Colors.white10),
        const Text('All Movies (VJ on Top Right)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        ...allMovies.map((m) => ListTile(leading: Stack(children: [CircleAvatar(backgroundColor: m.color, child: const Icon(Icons.movie, color: Colors.white)), Positioned(right: 0, top: 0, child: Container(padding: const EdgeInsets.all(2), decoration: const BoxDecoration(color: Color(0xFF9C27FF), shape: BoxShape.circle), child: const Text('VJ', style: TextStyle(fontSize: 6))))]), title: Text(m.title), subtitle: Text('${m.category} • ${m.vjName}'), trailing: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFF9C27FF), borderRadius: BorderRadius.circular(6)), child: Text(m.vjName, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold))))),
        const SizedBox(height: 20),
